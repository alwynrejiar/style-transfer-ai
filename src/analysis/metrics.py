...
def _yules_k(word_freq: Counter, total_words: int) -> float:
    """Yule's K ΓÇô vocabulary richness independent of text length.
    Lower K ΓåÆ richer vocabulary.  Typical prose: 80-200.
    """
    if total_words == 0:
        return 0.0
...
def _simpsons_diversity(word_freq: Counter, total_words: int) -> float:
    """Simpson's Diversity Index ΓÇô probability two random words differ."""
    if total_words <= 1:
        return 0.0
...
def _brunet_w(total_words: int, vocab_size: int) -> float:
    """Brunet's W ΓÇô another length-independent vocabulary measure.
...
def _honore_r(total_words: int, vocab_size: int, hapax_count: int) -> float:
    """Honor├⌐'s R statistic ΓÇô rewards large vocabularies with many hapax legomena."""
    if total_words == 0 or vocab_size == 0:
        return 0.0
...
def extract_deep_stylometry(text):
    ...
    Features extracted:
    ΓöÇ Full POS tag distribution (17 Universal POS tags)
    ΓöÇ 50 function-word frequencies
    ΓöÇ Dependency tree depth (avg & max)
    ΓöÇ Sentence length distribution (mean / median / std / min / max)
    ΓöÇ Vocabulary richness (hapax, Yule's K, Simpson's D, Brunet's W, Honor├⌐'s R)
    ΓöÇ Average word length
    ΓöÇ Contraction rate
    ΓöÇ Passive voice ratio
    ΓöÇ Modal verb frequency
    ΓöÇ Sentence-starter POS distribution
    ΓöÇ Named-entity type distribution
    ΓöÇ Character trigram profile (top 25)
    ΓöÇ Word bigram profile (top 25)
    ΓöÇ Punctuation / quotation density
    ΓöÇ Question & exclamation ratios
    """
...
def calculate_style_similarity(profile_a, profile_b):
    ...
    1. **Cosine similarity** on the full feature vector (POS + function words +
       scalar features).
    2. **Burrows' Delta** ΓÇô the gold-standard distance in computational
       stylometry, using z-scores of the most-frequent-word frequencies.
    3. **N-gram overlap** (Jaccard) on character trigram keys.

    Returns a dict with individual scores and a combined score (0.0ΓÇô1.0,
    ...
    # Convert to similarity (0-1); delta of 0 ΓåÆ similarity 1
...
def analyze_text_statistics(text):
    ...
    punctuation_counts = {
        ...
        'dashes': text.count('ΓÇö') + text.count('--'),
